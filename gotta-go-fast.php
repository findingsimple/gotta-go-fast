...
class WCS_Gotta_Go_Fast {
	...
	/**
	 * remove the all action
	 * @link https://support.woothemes.com/hc/en-us/articles/205214466-Subscriptions-2-0-Query-Monitor-Warning-The-all-action-is-extremely-resource-intensive-
	 */
	public function wcs_remove_deprecation_handlers() {
		add_filter( 'woocommerce_subscriptions_load_deprecation_handlers', '__return_false' );
	}


	/**
	 * Replaces the ActionScheduler_Logger's filter comment queries with this one
	 * @param  WP_Query object $query standard WordPress query
	 * @return void changes the query in place
	 */
	public function the_magic( $query ) {
	...
	/**
	 * Instead of joining with the wp_posts table (which can be massive) we should be able to just exclude comment types we don't need to include.
	 * Hooked from self::the_magic
	 *
	 * @param  array $clauses SQL clauses
	 * @param  WC_Query object $query standard WordPress Query object
	 */
	 ...
	 */
	public function pmgarman_unmoderated_comment_counts( $stats, $post_id ) {
		global $wpdb;
